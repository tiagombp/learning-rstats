...
# dados coletados diretamente dos labels do gráfico original
val <- rev(c(464,557,630,747,837,940,1036,1167,1290,1293,1331,1359))
ano <- 2018:2007
# ipca do ipeadata para 2007 a 2017 -- zero para 2018, pois vai ser o ano-base
ipca <- rev(c(4.46, 5.90, 4.31, 5.91, 6.50, 5.84, 5.91, 6.41, 10.67, 6.29, 2.95, 0))
names(ipca) <- ano

dados <- data.frame("desp_nominal" = val, ano, ipca)

desp_bahia <- dados %>%
  mutate(fator     = (1 + ipca/100),
         deflator  = cumprod(fator),
         desp_real = desp_nominal * deflator) %>%
  gather(desp_real, desp_nominal, key = "var", value = "valor")

ggplot(desp_bahia, aes(y = valor, fill = var, x = as.character(ano))) + 
  geom_col(position = "dodge2", width = 0.7) +
  scale_y_continuous(labels = function(x) {format(x, big.mark = ".", decimal.mark = ",")}) +
  scale_fill_manual(values = c("desp_nominal" = "goldenrod", 
                               "desp_real" = "#0885A6"),
                    labels = c("desp_nominal" = "Valores nominais", 
                               "desp_real" = "Valores reais (preços de 2018)")) +
  labs(x = NULL,
       y = NULL,
...
ggsave(filename = "graf_bahia_educ.png", width = 7, height = 5)
